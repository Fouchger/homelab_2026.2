#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Filename: ansible/roles/mikrotik_integration/files/notify.sh
# Created: 2026-01-19
# Description: Standalone, opt-in alerting helper for homelab_2026.2.
# Usage:
#   source /opt/homelab/mikrotik/notify.sh
#   notify_event "mikrotik" "ERROR" "Health check failed" '{"failures":["dns01"]}'
# Developer notes:
#   - Always appends an alert line to /root/.config/homelab_2026_2/alerts/alerts.log
#   - Optional channels:
#       * Webhook: set ALERT_WEBHOOK_URL (expects JSON POST)
#       * SMTP: set ALERT_SMTP_TO and ensure sendmail exists (msmtp provides it)
#   - Do not store credentials in state.env. If SMTP auth is required, export
#     SMTP_PASSWORD at runtime (prefer Vaultwarden).
# -----------------------------------------------------------------------------
...
notify_event() {
  ...
  alert_dir="/root/.config/homelab_2026_2/alerts"
